...
* 停止顺序:**业务线程先 join → 再** **`app().quit()`**。

* **在飞 HTTP 语义(v5.1 修正:graceful 边界)**:drogon `quit()` = 事件循环退出——停止接收新连接、当前迭代收敛;**已挂起的协程(`co_await RunOnPool`/上传流/打包流)不会被再调度,直接丢弃**;`RunOnPool` 工作池不等待(进程退出中止)。故在飞请求收尾由**业务层保障**:要么放自己守护线程(`Shutdown()` join = 真等待),要么设计为可中断+断点(上传落盘临时文件后 rename 等)。

* **验收**:Stop 无崩溃、无 UAF、无内存泄漏;业务线程 join 顺序正确;业务级在飞工作不依赖框架等待。
...
10. 优雅关闭:无崩溃/泄漏;业务线程先 join;**在飞 HTTP 由业务层保障**(守护线程/断点),框架不等待
