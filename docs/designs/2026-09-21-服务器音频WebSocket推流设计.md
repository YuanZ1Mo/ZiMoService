...
1. **推播(Push)**:采集每 100ms 产一片,立即向所有在线连接推送,稳态端到端延迟 ≤400ms(局域网手机;口径见 §6.2);
...
任何一次重定基(首次建连、断网重连、暂停恢复、重建)都只做一件事:**从直播边缘回退 `kBackfillSegs`(3 片 ≈ 300ms)起发**。即 `meta{baseSeq}` 之后紧跟 init 段与这一个补发批(≤ `kBackfillSegs` 片),随后转实时推送。

> **为什么只补 300ms、不补历史**:① 补发按片花时间(每片一次 append,手机上约 5~20ms),断网 60 秒后补 600 片要 6~12 秒才追得上,而这些内容恢复后又会立刻被规则② 跳到直播边缘丢弃 —— 越补越慢、也没有任何听觉收益;② 要的是"点击播放/恢复即近乎实时",不是"补听错过的那段"。所以服务端不保留可供回头的历史(§4.9 的缓冲窗口只有 1 秒),客户端也不会去要。
>
> 这 300ms 是必需的:媒体元素手里没有几百毫秒数据就不会进入播放态(实测 0.12 秒时停在 HAVE_METADATA:界面显示"正在播放"却不出声)。它不构成"补听",之后播放头一直贴在边缘上。
...
void ZmServerAudioStreamModule::OnFrame(const uint8_t* data, size_t len, uint64_t ptsMs)
{
    ...
    std::shared_ptr<const std::vector<uint8_t>> frame;                            // 帧体一次组装、多连接共享
    {
        ...
        while (m_segments.size() > kWindowSegments)   // 1s 起播缓冲
...
| 常量 | 值 | 用途 |
| --- | --- | --- |
| `kSegmentMs` | 100 | 片长(取自封装器) |
| `kWindowSegments` | 10 | 起播缓冲窗口(1s):只服务新连接的起播补发,不保留可供回头的历史 |
...
| `kBackfillSegs` | 3 | 起播补发片数(真机起播进不了播放态则上调到 4~5,代价是落后量 +100~200ms) |
...
| **保留** | 采集器、封装器、`EnsureReady`、起播缓冲(窗口由 600 片改为 10 片)、`OnTick` 停采判定(判据换 `m_wsClients`)、`OnCaptureDone`、`Authorize`(抽出按 cookie + ip 的形式)、`/status` |
...
| 自适应缓冲余量 `_leadSec` | 触发源由"拉片 RTT"改"帧到达间隔的抖动":`jitterMs = EWMA(|帧间隔 − segmentMs|,α=0.2)`,`余量 = max(_leadSec, jitterMs×2 + 0.15s, 3 片)`;翻倍/衰减/上限不变(0.3s 起,封顶 2s,45s 无卡顿后减半) |
...
> 两项时间指标的口径:从"客观条件就绪"到"真的出声" —— 断网一项从网络恢复计时,暂停一项从点击继续计时,都含重连(如需)、鉴权、补发 3 片与起播定位。实测预期 1~2 秒,数值是验收上界(超了判不通过),不是目标值;与"稳态端到端延迟"(你说的"差多远")是两个不同的量。
...
| 服务重启瞬间 | 短暂断音 | 重连后直接回直播边缘;起播补发的 3 片保证立刻有料可播 |
