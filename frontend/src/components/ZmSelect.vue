...
<template>
  ...
  <Teleport to="body">
    <div v-if="open" ref="pop" class="menu zsel-pop" role="listbox"
      ...
      <button v-for="(o, i) in options" :key="String(o.value)" type="button"
              class="menu-item zsel-opt" :class="{ on: i === active, picked: o.value === modelValue }"
              :disabled="o.disabled" role="option" :aria-selected="o.value === modelValue"
              @click="pick(o)" @mousemove="active = i">
        <span class="zsel-tick">
          <svg v-if="o.value === modelValue" width="14" height="14" viewBox="0 0 24 24" fill="none"
